...
exports.packageDarwin = function(project_path, project_name, project_version, company_name, license_path, manual_path, readme_path, script_path) {
  ...
  return new Promise(async function(resolve, reject) {
    ...
    await utils.asyncExec('"' + whiteboxPackages + '"', ['"' + packagesProj + '"'], {
      "shell": true
    });
...
function runInnoSetup(project_path, normal, legacy) {
  return new Promise(async function(resolve, reject) {
    ...
    if (normal) {
      ...
      let result = await utils.asyncExec('"' + inno + '"', ['"' + file + '"'], {
        "shell": true
      });
      console.log(result);
    }
    ...
    if (legacy) {
      ...
      let result = await utils.asyncExec('"' + inno + '"', ['"' + legacyFile + '"'], {
        "shell": true
      });
      console.log(result);
    }
